...
public class UniqueAccessPerformanceTest {
...
public static final CachePerformance cache = new CachePerformance((long)2<<34, (long)2<<31);
	...
	public static void main(String args[]) throws IOException
	{
		...
		CachePerformance.AccessThread[] threads = new CachePerformance.AccessThread[numOfKeys];
		...
		for(int i =0; i<threads.length; i++)
		{
			threads[i] = cache.getAccessThread(keys[i], entry);
			threads[i].start();
		}
		for(int i = 0; i<threads.length; i++)
		{
