...
public class UniqueRemovalPerformanceTest {
...
public static final CachePerformance cache = new CachePerformance((long)2<<34, (long)2<<31);
	...
	public static void main(String args[]) throws IOException
	{
		...
		CachePerformance.RemoveThread[] threads = new CachePerformance.RemoveThread[numOfKeys];
		...
		for(int i =0; i<threads.length; i++)
		{
			threads[i] = cache.getRemoveThread(keys[i], entry);
			threads[i].start();
		}
		for(int i = 0; i<threads.length; i++)
		{
